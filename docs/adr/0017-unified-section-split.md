...
**修订（2026-08-11）**：尺寸阈值从 1.3/1.5in 下调为 **0.4/0.4in**，并补测试锁定（`tests/test_decor_utils.py`）。

原阈值（宽 < 1.3in 且 高 < 1.5in）会误删真实题目图——实测两个物理讲义 313 张图发现：选项表格内的小尺寸题目图（最小 0.479×0.667in）被批量当作装饰图标清除（第 1 讲误删 10 张，含 alt 为 `test` 的 `image16.png` 线圈同心示意图；电路改文档还会误删约 20 张空 alt 电路图）。

新阈值依据实测分布（装饰图标显示尺寸恒为 0.194×0.194in，真实图最小 0.479×0.667in，中间无图）：图标侧 2 倍余量、真实图侧 1.2 倍余量。已知局限：同一图标被 Word 拉伸显示（表格内 1.222×1.472in）时无法仅凭显示尺寸识别，接受漏删（保留比误删安全）。
