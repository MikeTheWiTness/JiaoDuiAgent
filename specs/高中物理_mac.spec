...
"""高中物理 v3.0 PyInstaller spec（macOS .app）"""
import os
import shutil
import sys
...
if not pandoc_bin:
    raise SystemExit("未找到 pandoc，请先安装（brew install pandoc）")
...
a = Analysis(
    ...
    datas=[
        ...
        # 内置 pandoc（find_pandoc 从 _MEIPASS/bin 定位，目标机无需安装）
        (pandoc_bin, 'bin'),
    ],
    hiddenimports=[
        'core',
...
# macOS .app 打包（BUNDLE 仅 macOS 生效，Windows/Linux 忽略）
app = BUNDLE(
    coll,
    name='高中物理.app',
    icon=None,
    bundle_identifier='com.jiaodui.gaozhongwuli',
    info_plist={
        'CFBundleName': '高中物理校对',
        'CFBundleDisplayName': '高中物理校对工具',
