...
class IndependentSolveParams(BaseModel):
    question_without_answer: str = Field(
        description="去掉题目答案和解析的纯题目文本。必须确保上下文中不包含任何答案或解析内容，"
                    "以保证独立解题不受已见答案污染。"
    )
    solve_prompt: str = Field(
        description="解题指令 prompt。由主 agent 生成，包含解题要求、格式规范等。"
    )
    original_answer: str | None = Field(
        default=None,
        description="（可选）题目原始答案，用于写入 _物理求解.md 的答案比对段落。"
    )
...
class IndependentSolveTool(BaseTool):
    ...
    description: str = (
        "独立解题工具：将去答案题目 + 解题 prompt 送入干净上下文（无主对话历史），"
        "发起单次 API 求解，返回独立答案。"
        "用于难题答案校验——主 agent 拿到独立答案后与题目答案比对，判断一致性。"
        "仅在判定为难题（多过程/电磁场/能量动量转化/复杂受力）时调用。"
    )
    ...
    def _run(
        ...
        Args:
            question_without_answer: 去答案纯题目
            solve_prompt: 解题指令
            original_answer: 题目原始答案（可选，用于落盘比对）
        ...
        messages = [
            ...
            {"role": "user", "content": f"请独立求解以下物理题目（不要依赖任何外部答案）：\n\n{question_without_answer}"},
        ]
    ...
    def _write_physics_solution(
        ...
        try:
            ...
            if original_answer:
                lines.append("\n\n---\n")
                lines.append("## 答案比对\n")
                lines.append("- **题目答案**：" + original_answer[:500] + "\n")
